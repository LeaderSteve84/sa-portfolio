...
class ReferenceForm(FlaskForm):
    ...
    reference_type = SelectField('Select Type', choices=[
        ('testimony', 'Testimony'), ('quote', 'Quote'), ('feedback', 'Feedback')
    ], validators=[DataRequired()])
    message = TextAreaField('Message (minimum of 10 characters)', validators=[DataRequired(), Length(min=10)])
    reference_link = URLField('URL_Link (Optional)', validators=[Optional()])  #, URL()])
    contact = StringField('Email | Mobile Contact (Optional)', validators=[Optional(), Length(max=100)])
