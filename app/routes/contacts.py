...
def create_contact_message():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("Your message has been successfully submitted. We'll get back to you soon. Thank You!", 'success')
        return redirect(url_for('main.contacts.create_contact_message'))
    else:
        if form.errors != {}:
            for error_message in form.errors.values():
                flash('There is an error creating your contact message', 'error')
...
def view_contact_messages():
        ...
        return render_template('view_contact_messages.html', contact_messages=contact_messages)


@contacts_bp.route("/contact/<string:message_id>/edit", methods=['GET', 'POST'], strict_slashes=False)
def update_contact_message(message_id):
        ...
        if form.validate_on_submit():
            ...
            if form.errors != {}:
                for error_message in form.errors.values():
                    flash('There is an error updating your contact message: {error_message}', 'error')
        return render_template('update_contact_message.html', form=form, contact_message=contact_message)


@contacts_bp.route("/contact/<string:message_id>/delete", methods=['POST'], strict_slashes=False)
